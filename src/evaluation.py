...
def outbreak_metrics(
    y_true: np.ndarray,
    y_pred: np.ndarray,
    percentile: float = 95.0,
    threshold: float | None = None,
) -> dict:
    # REVIEW FIX: the outbreak threshold must come from TRAINING data, not from the
    # test ground truth. Pass `threshold` (computed on train) at call sites. If it
    # is None we fall back to the test percentile, which leaks and is kept only for
    # backward compatibility with the pre-review runs.
