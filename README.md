...
- Copy tất cả file tỉnh vào `data/raw/` (vd: `squeezed_Hà Nội.xlsx`).
- Mỗi file cần có tối thiểu các cột:
  - `year`, `month`, `Dengue_fever_rates`
  - climate vars và socio vars theo `src/config.py`
...
## 6) Checklist thực thi (assign team)
...
| 3 | Core model | XGB / HistGB / LSTM | Global pooled | XGB(300,6,0.05), LSTM(seq=12, hidden=64) | ML + DL | 9 | tốt hơn baseline 5–10% | Global học được pattern chung |
